...
# Map pool and pick/ban structure
MAP_POOL = [
...
class PickBanSession:
    ...
    def make_choice(self, action, player, map_name):
        ...
        if not self.is_map_available(map_name):
            return False, f"{map_name} is not a valid or available map."
...
@tree.command(name="start_pickban", description="Start a pick/ban session between two players")
...
async def start_pickban(interaction: discord.Interaction, player1: discord.Member, player2: discord.Member):
    ...
    await interaction.response.send_message(f"🎮 Map pick/ban started between {player1.mention} and {player2.mention}.")
...
@app_commands.describe(map_name="Map to pick")
